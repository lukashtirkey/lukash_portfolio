...
*   `resume.md` - A clean, plain-text Markdown-formatted resume ideal for quick parsing or updating GitHub profiles.
*   `package.json` - Node.js configuration defining the start script (`npm start` / `yarn start`) for Render and cloud deployments.
*   `server.js` - Standalone zero-dependency HTTP server to serve portfolio assets on cloud services like Render.
